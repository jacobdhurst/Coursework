...
def euler_forward(A, y, ht):
    ...
    '''
    g = 0
    return y + ht*A*y + ht*g #(speye(A.shape[0], format='csc') - ht*A)*y + ht*g #y + 0.0 # Fill in with correct values! 
...
def RK2(A, y, ht):
    ...
    '''
    g = 0
    k1 = A*y + g # Fill in with correct values!
    k2 = (1+ht)*(A*y + g) # Fill in with correct values! 
    return y + (ht/2.)*(k1+k2) # Fill in with correct values! 
...
#
# u(t,x,y) = 0,       for (x,y) on the boundary [g]
#
# u(t=0,x,y) = sin(pi x) * sin(pi y)            [j]
#
##
...
t0 = 0.0
T = 1.0
nt = 1001 #101 for backward, 1001 for rk2
ht = (T - t0)/float(nt-1)
